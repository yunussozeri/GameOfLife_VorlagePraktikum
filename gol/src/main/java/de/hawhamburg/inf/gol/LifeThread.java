...
public class LifeThread extends Thread {
    ...
    /**
     * Sets the interrupted flag, so that this thread stops after finishing the
     * current task.
     */
    @Override
    ...
    public void run() {
        while (!isInterrupted) {
            try {
                ...
                Logger.getLogger(LifeThread.class.getName()).log(Level.SEVERE, "ZORTLADIN", ex);
                //ensure the thread stops after exception catched!
                interrupt();
            }
        }
    }
